---
title: Applying image effects and filters
---
This document describes how image effects and filters are applied to an image based on user selection. The flow coordinates user interaction and effect processing, determining whether configuration is needed and updating the image accordingly.
...
In `Process_EffectsMenu`, we kick off the effect dispatch logic. The function checks the processID against a list of supported effects, and for each match, it either pops up a dialog for user input or runs the effect directly. Calling Modules/Interface.bas next lets us show dialogs or trigger UI updates, which is needed for interactive effects.
...
Back in `Process_EffectsMenu`, after handling dialogs/UI, we hit special cases like 'Grid blur'. These legacy or planned effects are routed to Modules/Filters_Area.bas, which actually runs the filter logic. This keeps old features working and lets us add new ones later.
...
## Applying Grid-Based Blur

```mermaid
%%{init: {"flowchart": {"defaultRenderer": "elk"}} }%%
flowchart TD
    node1["Prepare image data for grid blur"]
    click node1 openCode "Modules/Filters_Area.bas:282:284"
    node1 --> loop1
    subgraph loop1["For each column in selected image area"]
        node2["Sum color values for each vertical line"]
        click node2 openCode "Modules/Filters_Area.bas:314:327"
    end
    loop1 --> loop2
    subgraph loop2["For each row in selected image area"]
        node3["Sum color values for each horizontal line"]
        click node3 openCode "Modules/Filters_Area.bas:330:343"
    end
    loop2 --> loop3
    subgraph loop3["For each column in selected image area"]
        node4["For each pixel: Average vertical and horizontal values, cap at 255, assign new color"]
        click node4 openCode "Modules/Filters_Area.bas:348:366"
        node4 --> node5{"User cancels operation?"}
        click node5 openCode "Modules/Filters_Area.bas:369:369"
        node5 -->|"Yes"| node6["Exit blur operation"]
        click node6 openCode "Modules/Filters_Area.bas:369:369"
        node5 -->|"No"| node7["Update progress bar if needed"]
        click node7 openCode "Modules/Filters_Area.bas:368:371"
        node7 --> node4
    end
    loop3 --> node8["Finalize image data"]
    click node8 openCode "Modules/Filters_Area.bas:375:378"
...
In `FilterGridBlur`, we set up the pixel data and compute row/column sums for the blur, with Interface calls used for user feedback.
...
After Interface feedback, `FilterGridBlur` runs the main blur loop. For each pixel, it averages the vertical and horizontal color sums, clamps the result, and writes it back. ProgressBars calls are used to update the UI at smart intervals and handle user cancellation.
...
After ProgressBars updates, `FilterGridBlur` wraps up by unwrapping the pixel array and finalizing the image data. This commits the blur changes and cleans up memory.
...
## Continuing Effect Dispatch

```mermaid
%%{init: {"flowchart": {"defaultRenderer": "elk"}} }%%
flowchart TD
    node1["User selects an effect or filter"] --> node2{"processID: Which effect/filter?"}
    click node1 openCode "Modules/Processor.bas:1723:2014"
    node2 -->|"Known effect/filter"| node3{"raiseDialog: Show dialog for user input?"}
    click node2 openCode "Modules/Processor.bas:1723:2014"
    node2 -->|"No match"| node7["No effect applied"]
    click node7 openCode "Modules/Processor.bas:2023:2025"
    node3 -->|"Yes"| node4["Show dialog for selected effect"]
    click node3 openCode "Modules/Processor.bas:1725:2021"
    node3 -->|"No"| node5["Apply selected effect directly"]
    click node4 openCode "Modules/Processor.bas:1725:2021"
    click node5 openCode "Modules/Processor.bas:1725:2021"
    node4 --> node6["Return: Effect applied"]
    node5 --> node6
    click node6 openCode "Modules/Processor.bas:1726:2022"
classDef HeadingStyle fill:#777777,stroke:#333,stroke-width:2px;
```
...
After returning from Filters_Area.bas, `Process_EffectsMenu` keeps dispatching effects. For each effect, it checks if a dialog is needed and calls Interface to handle user input or runs the effect directly. This keeps all effect handling in one place.
...
At the end of `Process_EffectsMenu`, after all Interface calls, we handle special cases like Photoshop plugins. These use a different workflow, so we call a dedicated plugin handler instead of running the effect directly.
